...
```
gundamFitter -c super_advanced_gundam_example/config.yaml
```
...
mv mydataset.root super_advanced_gundam_example/inputs/datasets/mydataset.root
```

(The build macro writes `mydataset.root` to the current working directory.
Move it to `./super_advanced_gundam_example/inputs/datasets/` before running the fitter.)
